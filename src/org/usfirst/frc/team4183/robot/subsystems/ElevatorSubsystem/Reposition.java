...
public class Reposition extends Command {
	...
	public void execute() {
		/// TODO: Need to clean this up... the pneuma delay is only needed if we have
		/// them
		...
		if (targetPathPerc < 0) {
			...
			if (requestedPosition == -1) {
				...
				// to about 4 to 6 inches per second, which should allow the operator time to
				// visually
				// see and then respond to a the condition and successfully stop without
				// overshoot.
			...
			if (Robot.driveSubsystem.getPercentComplete(targetPathPerc) == DriveSubsystem.TrajectoryPercent.PASSED) {
				Robot.elevatorSubsystem.holdPosition(requestedPosition);
			} else if (Robot.driveSubsystem
					.getPercentComplete(targetPathPerc) == DriveSubsystem.TrajectoryPercent.FAULT) {
	...
	protected boolean isFinished() {
		...
		// Basically checks to see if the there is not any joystick movement or any
		// buttons pressed for the elevPositions
		...
		if (((requestedPosition == -1) && // Manual and inside deadband on joystick
				...
				|| ((Robot.elevatorSubsystem.getElevatorCurrent() > RobotMap.ELEVATOR_MAX_CURRENT) && // Current too
																										// high and
																										// going down
																										// manually
